...
interface MongoSerial<T> {
    @JvmDefault fun queryDocument(obj: T) = Document()
    ...
    companion object {
        ...
        @JvmStatic fun register(type: Class<*>, serial: MongoSerial<*>) {
        ...
        @JvmStatic fun <T> fromDocument(document: Document, type: Class<T>) =
            getSerializerThrowing(type).deserialize(document)

        @JvmStatic inline fun <reified T> fromDocument(document: Document) =
            fromDocument(document, T::class.java)

        @JvmStatic fun <T> toDocument(obj: T, type: Class<T>) =
            getSerializerThrowing(type).serialize(obj)

        @JvmStatic inline fun <reified T> toDocument(obj: T) =
            toDocument(obj, T::class.java)

        @JvmStatic fun <T> getQueryDocument(obj: T, type: Class<T>) =
            getSerializerThrowing(type).queryDocument(obj)

        @JvmStatic inline fun <reified T> getQueryDocument(obj: T) =
