...
public class RoleAddCommand extends Command {
    ...
    public Message execute(Source source, Message message, String[] args) {
        ...
        if (target.getRoles().contains(role)) {
            CriticalAlert alert = new CriticalAlert();
            alert.setTitle("Uh Oh!").setDescription("The user already has the role you are trying to add!");
            return new MessageBuilder(alert.build(user)).build();
        }
        ...
        sAlert.setDescription("You have successfully added the role `" + role.getName() + "` to " + fullTargetName + "!");
        ...
        if (log) {
            ...
            ColoredAlert cAlert = new ColoredAlert(SourceColor.GREEN);
            cAlert.setDescription("**Role Added By:** " + fullUserName + " (" + user.getId() + ")\n" +
                    "**Role Added To:** " + fullTargetName + " (" + targetUser.getId() + ")\n" +
                    "**Role Added:** " + role.getName() + " (" + role.getId() + ")\n" +
                    "**Reason:** " + reason);
